...
function ensureSampleTemplate() {
  ...
  // If we've already decided about seeding in the past, stop here.
...
— Your Name`,
  ...
  // Mark as decided so we never seed again in future loads.
  ...
  function createTemplate({ name, content, folderId = null }) {
    const store = getStore();
    const t = {
      id: uid(),
